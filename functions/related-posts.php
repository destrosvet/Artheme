...
function artalk_get_related_posts($post_id=null,$args='',$stop_tags=array(), $stop_cats=array()) {
  ...
  if ( ! absint($post_id) && is_main_query() ) {
	  if ( ! $post_id = get_the_ID() ) {
	  	if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'MISSING OR WRONG ID'); }
	    return false;
	  }
  }
  // Try cache
  if ( $load_from_cache && $cached_posts_ids = get_transient( 'artalk_related_posts_for_'.$post_id ) ) {
    if ( $cached_posts = get_posts( array( 'post__in' => $cached_posts_ids ) ) ) {
      if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'LOADING FROM CACHE'); }
      return $cached_posts;
    }
  }
  // Set shared args
  ...
  if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'LOADING BY TAGS'); }
  ...
  foreach ($post_tags as $key => $post_tag) {
  	if ( ! in_array($post_tag->name,$stop_tags) ) {
  		...
  		if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'ADDED TAG '.$post_tag->name); }
  	}
  }
  if ( $log ) { error_log('>>> artalk_get_related_posts(): '.count($post_tags_ids).'/'.count($post_tags).' TAGS VALID'); }
  ...
  if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'FOUND '.$related_posts_count.' POSTS BY TAGS'); }
  ...
  if ( $related_posts_count < $args['posts_per_page'] ) {
    ...
    foreach ($post_cats as $key => $post_cat) {
	  	if ( ! in_array($post_cat->name,$stop_cats) ) {
	  		if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'ADDED CAT '.$post_cat->name); }
    ...
    if ( $log ) { error_log('>>> artalk_get_related_posts(): '.'FOUND '.count($related_by_cat).' POSTS BY CAT'); }
  }
