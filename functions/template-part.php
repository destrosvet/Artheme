...
function more_post_ajax()
{
    ...
    while ($query->have_posts()) : $query->the_post();
        ...
        if (((artalk_in_artservis($terms) || artalk_get_current_category() == 'foto-report' || $taxonomy == 'post_tag' || $author_id) || ($search_string || $search_category || $search_tag !='slovensko')) and $cat != '3876') {
...
function get_revue_download()
{
    ...
    $output .= '<a href="' . get_permalink() . 'ar-' . $RevueCat[0] . '-cz-' . get_the_ID() . '/"></a><a href="' . get_permalink() . 'ar-' . $RevueCat[0] . '-en-' . get_the_ID() . '/"></a>';
    $output .= '</div>';
    echo $output;
}
