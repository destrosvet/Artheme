<?php
/**
 * Created by Teapot
 * User: Filip Uhlir
 * Date: 23.01.2017
 * Time: 14:27
 */
?>
<div class="nopadding nomargin" id="service-bottom">
    <div class="col-md-6 col-sm-12 col-xs-12 noleftpadding no-padding-md norightpadding service-b-mm" id="b-mm">
        <?php  do_action(artalk_service_part('Monitoring médií','Media monitoring','s-b-mm',true,5,1,'media_li')) ?>
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12 noleftpadding no-padding-md norightpadding service-b-top"" id="b-top">
        <?php  do_action(artalk_service_part('Doprovodné programy','Doprovodné programy','s-b-dp',true,5)) ?>
    </div>
</div>